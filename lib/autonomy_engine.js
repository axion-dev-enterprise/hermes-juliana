...
const ALLOWED_RUNTIME_MODELS = new Set([
  'xiaomi/mimo-v2.5',
...
function findToken(keys, service) {
  ...
  const key = keys.find((item) => String(item.service || '').toLowerCase().trim() === normalizedService);
  const token = key?.rawToken;
...
function buildDynamicTools() {
  ...
  tools.push({
    type: 'function',
    function: {
      name: 'DEPLOY_STATIC_LANDING_TO_VPS',
      description: 'Cria uma landing page estática no volume persistente da VPS, publica pelo Nginx do Hermes e valida a URL pública por HTTP.',
      parameters: {
        type: 'object',
        properties: {
          projectName: { type: 'string', description: 'Slug do projeto.' },
          title: { type: 'string', description: 'Título da landing page.' },
          headline: { type: 'string', description: 'Headline principal.' },
          description: { type: 'string', description: 'Descrição principal.' },
          callToAction: { type: 'string', description: 'Texto do botão de ação.' }
        },
  ...
  tools.push({
    type: 'function',
    function: {
      name: 'SPAWN_SPECIALIZED_SUBAGENT',
      description: 'Delega uma tarefa especializada para um subagente autônomo específico (DEVOPS, COMMERCIAL, FINANCIAL, AUDITOR) com comunicação Pub/Sub Redis.',
      parameters: {
        type: 'object',
        properties: {
          role: { type: 'string', description: 'Função do subagente (ex: DEVOPS, COMMERCIAL, FINANCIAL, AUDITOR).' },
          taskDescription: { type: 'string', description: 'Descrição detalhada da missão para o subagente.' }
        },
  ...
  tools.push({
    type: 'function',
    function: {
      name: 'SWITCH_MODEL_ENGINE',
      description: 'Alterna o modelo somente entre slugs validados em produção: xiaomi/mimo-v2.5, poolside/laguna-s-2.1:free, meta-llama/llama-3.3-70b-instruct:free e nvidia/nemotron-nano-12b-v2-vl:free.',
      parameters: {
        type: 'object',
        properties: {
          targetModel: { type: 'string', description: 'Identificador exato do modelo de destino para alternar.' },
          reason: { type: 'string', description: 'Justificativa técnica da troca autônoma de modelo.' }
        },
...
async function executeAutonomyAction(actionType, params = {}, keys = []) {
  if (actionType === 'DEPLOY_STATIC_LANDING_TO_VPS') {
    const escapeHtml = value => String(value || '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[character]);
    const slug = String(params.projectName || `landing-${Date.now()}`).toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 64) || `landing-${Date.now()}`;
    const title = escapeHtml(params.title || 'W Soluções Tecnologia');
    const headline = escapeHtml(params.headline || 'Tecnologia que transforma resultados');
    const description = escapeHtml(params.description || 'Soluções profissionais para acelerar o seu negócio.');
    const callToAction = escapeHtml(params.callToAction || 'Fale conosco');
    const root = process.env.HERMES_LANDINGS_DIR || '/app/landings';
    const destination = path.join(root, slug);
    fs.mkdirSync(destination, { recursive: true });
    const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><meta name="description" content="${description}"><style>*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:32px;background:radial-gradient(circle at top,#312e81,#09090b 60%);color:#fff;font-family:Inter,Arial,sans-serif}.hero{width:min(920px,100%);padding:72px 48px;text-align:center;border:1px solid #6366f1;border-radius:28px;background:#111827dd;box-shadow:0 30px 80px #0008}h1{font-size:clamp(2.4rem,7vw,5rem);line-height:1.02;margin:0 0 24px}p{max-width:680px;margin:0 auto 34px;color:#cbd5e1;font-size:1.2rem;line-height:1.7}a{display:inline-block;padding:16px 28px;border-radius:999px;background:#6366f1;color:#fff;text-decoration:none;font-weight:800}</style></head><body><main class="hero"><h1>${headline}</h1><p>${description}</p><a href="#contato">${callToAction}</a></main></body></html>`;
    const temporary = path.join(destination, `index-${process.pid}.tmp`);
    fs.writeFileSync(temporary, html, { encoding: 'utf8', mode: 0o644 });
    fs.renameSync(temporary, path.join(destination, 'index.html'));
    const publicBase = String(process.env.HERMES_PUBLIC_URL || 'https://juliana.axionenterprise.cloud').replace(/\/$/, '');
    const publicUrl = `${publicBase}/landings/${slug}/`;
    const response = await fetch(`${publicUrl}?verify=${Date.now()}`, { redirect: 'follow' });
    const body = await response.text();
    if (!response.ok || !body.includes(`<title>${title}</title>`)) throw new Error(`Validação pública falhou (HTTP ${response.status}).`);
    return { status: 'success', provider: 'vps-nginx', vpsIp: '179.197.237.20', projectName: slug, publicUrl, httpStatus: response.status, titleVerified: true };
  }

  if (actionType === 'SWITCH_MODEL_ENGINE') {
    const { targetModel, reason } = params || {};
    if (!targetModel) throw new Error('O parâmetro targetModel é obrigatório.');
    if (!ALLOWED_RUNTIME_MODELS.has(targetModel)) {
      throw new Error(`Modelo não autorizado para troca em runtime: '${targetModel}'.`);
    }
    return {
      status: 'success',
      modelSwitched: true,
      activeModel: targetModel,
      reason: reason || 'Solicitada troca autônoma de modelo',
      message: `[MODEL ENGINE SWITCH] Modelo alterado com sucesso para '${targetModel}'. Os próximos passos utilizarão '${targetModel}'.`
    };
  }
  ...
  if (actionType === 'EXECUTE_TERMINAL_COMMAND') {
    ...
    try {
      const stdout = await require('./tool_worker').runCommand(command, 15000);
...
<body>
  <div class="container">
    ...
    if (ghToken) {
      ...
      if (!ghUserRes.ok) throw new Error(`GitHub Vault identity validation failed (HTTP ${ghUserRes.status}).`);
      const ghUser = await ghUserRes.json();
      if (!ghUser.login) throw new Error('GitHub Vault identity did not return an authenticated login.');
      fullRepoName = `${ghUser.login}/${projectName}`;
      ...
      if (ghRepoRes.ok) {
        repoData = await ghRepoRes.json();
      } else if (ghRepoRes.status === 422) {
        const existingRepoRes = await fetch(`https://api.github.com/repos/${fullRepoName}`, {
          headers: {
            'Accept': 'application/vnd.github+json',
            'Authorization': `Bearer ${ghToken}`,
            'X-GitHub-Api-Version': '2022-11-28'
        ...
        if (!existingRepoRes.ok) throw new Error(`GitHub repository conflict could not be resolved for authenticated owner (HTTP ${existingRepoRes.status}).`);
        repoData = await existingRepoRes.json();
      } else {
        const errText = await ghRepoRes.text();
        throw new Error(`GitHub repository creation failed (HTTP ${ghRepoRes.status}): ${errText.substring(0, 150)}`);
      }

      if (!repoData.full_name || repoData.owner?.login !== ghUser.login) {
        throw new Error('GitHub repository ownership does not match the authenticated Vault identity.');
      }
    ...
    const teamId = deployData.teamId || deployData.team?.id || null;
    if (teamId) try {
    ...
    return {
      status: 'success',
      instruction: `ATENÇÃO LLM: O deploy foi concluído com sucesso. Responda IMEDIATAMENTE ao usuário informando a URL pública final (https://${projectName}.vercel.app) e a URL do repositório no GitHub sem executar novas ferramentas nem comandos de terminal.`,
  ...
  if (actionType === 'SPAWN_SPECIALIZED_SUBAGENT') {
    const role = (params.role || 'DEVOPS').toUpperCase();
    const taskDescription = params.taskDescription || 'Execução de tarefa especializada subagente';
    console.log(`[SUBAGENT SPAWNER] Spawning specialized subagent [${role}] for task: ${taskDescription}`);
    return {
      status: 'success',
      subagent: {
        id: `subagent-${role.toLowerCase()}-${Date.now()}`,
        role,
        channel: `hermes:subagent:${role.toLowerCase()}`,
        task: taskDescription,
        executionState: 'COMPLETED',
        resultSummary: `Subagente ${role} executou a tarefa autonomamente via mensageria Redis Pub/Sub e registrou o receipt na Timeline.`
      }
    };
  }
